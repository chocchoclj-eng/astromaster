/* scripts/chart-worker.cjs */
const path = require("path");
const sweph = require("sweph");

function norm(x){ return ((x % 360) + 360) % 360; }

function localToUTC(y,m,d,hh,mm,ss,tz){
...
function getHousesKoch(jd_ut, lat, lon){
...
function calcChartKoch(input){
  ...
  // Swiss Ephemeris body IDs (fixed numeric constants)
  const bodiesMap = {
    Sun: 0,
    Moon: 1,
    Mercury: 2,
